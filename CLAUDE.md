...
## The schema is pushed at boot

`src/index.ts` calls `syncSchema` (`src/db/sync.ts`) before the server loads, so **a schema change needs no manual step**: drizzle-kit's `pushSchema` diffs `src/db/schema.ts` against the database and applies whatever the two disagree on, inside one transaction held under a Postgres advisory lock. A database that already matches costs one introspection and nothing else.

That makes `drizzle-kit` a runtime dependency, not just a dev one. There are no migration files; `bun run db:push` is the same push by hand.
